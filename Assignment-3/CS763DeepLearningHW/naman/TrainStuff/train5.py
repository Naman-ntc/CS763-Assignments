...
def Try_em_all():
	...
	for reg in par_regularization:
		...
		for bs in batchSize:
			reg = Oldreg
			learningRate = 3e-2
			stringg = "Model5||reg="+str(reg)+"||bs="+str(bs)+".txt"
			import sys
			sys.stdout = open(stringg,'w') 
			model = Model()	
			model.addLayer(Linear(108*108, 700))
			#model.addLayer(BatchNorm(800))
			model.addLayer(ReLU())
			model.addLayer(Linear(700, 80))
			...
			iterations_count = 128*10000//bs
			lr_decay_iter = iterations_count//9
			...
			if (trainAcc(model) > bestAcc):	
				torch.save(model,open("model5.model",'wb'))
